...
class TestDataset(Dataset):
    def __init__(self, data_path):
        ...
        for data in sorted(datas):
            data_name = data.split('/')[-1]
            # if data_name == 'warp1' or data_name == 'warp2' or data_name == 'mask1' or data_name == 'mask2' or data_name == 'remain_input1' or data_name == 'remain_input2':
            if data_name == 'warp1' or data_name == 'warp2' or data_name == 'mask1' or data_name == 'mask2':    
                self.datas[data_name] = {}
                self.datas[data_name]['path'] = data
                self.datas[data_name]['image'] = glob.glob(os.path.join(data, '*.jpg'))
    ...
    def __getitem__(self, index):
        ...
        # load remain_input1
        # remain_input1 = cv2.imread(self.datas['remain_input1']['image'][index])
        # remain_input1 = remain_input1.astype(dtype=np.float32)

        # load remain_input2
        # remain_input2 = cv2.imread(self.datas['remain_input2']['image'][index])
        ...
        # return (warp1_tensor, warp2_tensor, mask1_tensor, mask2_tensor, remain_input1_tensor, remain_input2_tensor)
        return (warp1_tensor, warp2_tensor, mask1_tensor, mask2_tensor)
