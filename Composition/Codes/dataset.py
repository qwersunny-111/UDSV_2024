...
class TestDataset(Dataset):
    def __init__(self, data_path):
        ...
        for data in sorted(datas):
            data_name = data.split('/')[-1]
            if data_name in ['warp1', 'warp2', 'mask1', 'mask2', 'remain_input1', 'remain_input2']:
                self.datas[data_name] = {}
                self.datas[data_name]['path'] = data
                self.datas[data_name]['image'] = glob.glob(os.path.join(data, '*.jpg'))
                self.datas[data_name]['image'].sort()
    ...
    def __getitem__(self, index):
        # load image1
        warp1_path = self.datas['warp1']['image'][index]
        ...
        # load image2
        warp2_path = self.datas['warp2']['image'][index]
        ...
        # load mask1
        mask1_path = self.datas['mask1']['image'][index]
        ...
        # load mask2
        mask2_path = self.datas['mask2']['image'][index]
        ...
        # load remain_input1
        remain_input1_path = self.datas['remain_input1']['image'][index]
        remain_input1 = cv2.imread(remain_input1_path)
        remain_input1 = remain_input1.astype(dtype=np.float32)

        # load remain_input2
        remain_input2_path = self.datas['remain_input2']['image'][index]
        remain_input2 = cv2.imread(remain_input2_path)
        remain_input2 = remain_input2.astype(dtype=np.float32)
        ...
        mask1_tensor = torch.tensor(mask1)
        mask2_tensor = torch.tensor(mask2)
        remain_input1_tensor = torch.tensor(remain_input1)
        remain_input2_tensor = torch.tensor(remain_input2)

        # Extract image names
        warp1_name = os.path.basename(warp1_path)
        warp2_name = os.path.basename(warp2_path)

        return (warp1_tensor, warp2_tensor, mask1_tensor, mask2_tensor, remain_input1_tensor, remain_input2_tensor, warp1_name, warp2_name)
