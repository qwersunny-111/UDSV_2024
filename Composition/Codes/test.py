...
def test(args):
    ...
    for i, batch_value in enumerate(test_loader):
        ...
        remain_input1_tensor = batch_value[4].float()
        remain_input2_tensor = batch_value[5].float()
        warp1_name = batch_value[6][0]
        warp2_name = batch_value[7][0]
        ...
        cv2.imwrite(os.path.join(path_learn_mask1, warp1_name), learned_mask1)
        cv2.imwrite(os.path.join(path_learn_mask2, warp2_name), learned_mask2)
        cv2.imwrite(os.path.join(path_final_composition, warp1_name), stitched_image)
        cv2.imwrite(os.path.join(path_final_composition_all, warp1_name), cv2.hconcat([remain_input1, stitched_image_cropped, remain_input2]))
