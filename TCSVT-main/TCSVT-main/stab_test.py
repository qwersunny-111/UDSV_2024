...
class MeshFlowStabilizer:
    ...
    '''
    indicating which definition to use for the energy function's adaptive weights.
    ...
    * ADAPTIVE_WEIGHTS_DEFINITION_CONSTANT_LOW: Set the adaptive weights to a constant low value.
    ...
    ADAPTIVE_WEIGHTS_DEFINITION_CONSTANT_HIGH_VALUE = 50
    ADAPTIVE_WEIGHTS_DEFINITION_CONSTANT_LOW_VALUE = 2

    ##  初始化参数  ##
    def __init__(self, mesh_row_count=mesh_row_count, mesh_col_count=mesh_col_count,  # 网格行数与列数，定点数各加1
                 mesh_outlier_subframe_row_count=3, mesh_outlier_subframe_col_count=4,  # 图像划分为4*4，设立局部阈值RANSAC
    ...
    def stabilize(self, input_path, output_path, adaptive_weights_definition = ADAPTIVE_WEIGHTS_DEFINITION_ORIGINAL):
        ...
        * adaptive_weights_definition: Which method to use for computing the energy function's adaptive weights.
    ...
    def _get_unstabilized_frames_and_video_features(self, input_path):
        ...
        with tqdm.trange(num_frames) as t:
            ...
            for frame_index in t:
                ...
                if success:
                    # 规范分辨率
                    unstabilized_frame = cv2.resize(pixels, (960,540))
                else:
                    print('capture error')
                    exit()
    ...
    def _get_matched_features_and_homography(self, early_frame, late_frame):
        ...
        # feature_in_bbox = []             # BBox内特征点的索引 运动目标检测？
    ...
    ##  LoFTR方法获取局部网格内的特征点坐标 仅用于拼接 此处弃用  ##
    def _loftr_in_subframe(self, early_subframe, late_subframe, subframe_offset):
...
##  调用多进程 根据四个顶点稳定前后的运动向量之差计算单应矩阵并进行网格变形  ##
def get_stabilized_frames_and_crop_boundaries_with_multiprocessing(num_frames, unstabilized_frames, vertex_unstabilized_displacements_by_frame_index, vertex_stabilized_displacements_by_frame_index):
    ...
    job = partial(multiprocessing_job_for_warp, unstabilized_frames, stabilized_motion_mesh_by_frame_index)
...
def multiprocessing_job_for_warp(unstabilized_frames, stabilized_motion_mesh_by_frame_index, index):
    # unstabilized_vertex_x_y and stabilized_vertex_x_y are CV_32FC2 NumPy arrays
    # of the coordinates of the mesh nodes in the stabilized video, indexed from the top left
    # corner and moving left-to-right, top-to-bottom.
    total_frames = len(unstabilized_frames)
...
def plot_vertex_profiles(origin_paths_1, stabilized_paths): # , origin_paths_2
    """
    ...
    for i in range(0, origin_paths_1.shape[1]):
        for j in range(0, origin_paths_1.shape[2], 10):
            plt.plot(origin_paths_1[:, i, j, 1], label='original trajectory')
            # plt.plot(origin_paths_2[:, i, j, 0])
            plt.plot(stabilized_paths[:, i, j, 1],label='stabilized trajectory')
            plt.xlabel("Frame")
            plt.ylabel("Pixel")
            plt.legend(loc='best')
            plt.grid(visible=True ,color = 'k', linestyle = '--', linewidth = 0.5)
            plt.savefig('pic/stabilize_path_1/stabilized'+str(i)+'_'+str(j)+'.png')
            plt.clf()
#画图
...
if __name__ == '__main__':
    # main()
    # input_path = '20230502/0506_3_l_300_stab.mp4'
    # output_path = '20230502/0506_3_l_300_stabilized.avi'
    input_path = 'experiments/videos/case3.mp4'
    output_path = 'experiments/case5/case3_our.avi'
    stabilizer = MeshFlowStabilizer(visualize=True)
    ...
    # 取图像、总帧数、帧率、编码方式
    unstabilized_frames, num_frames, frames_per_second = stabilizer._get_unstabilized_frames_and_video_features(input_path)
    # 获得图像网格顶点初始的运动向量
    vertex_unstabilized_displacements_by_frame_index, homographies = stabilizer._get_unstabilized_vertex_displacements_and_homographies(
        num_frames, unstabilized_frames)
    # 获得图像网格顶点平滑后的运动向量
    vertex_stabilized_displacements_by_frame_index = stabilizer._get_stabilized_vertex_displacements(
        num_frames, unstabilized_frames, adaptive_weights_definition,
        vertex_unstabilized_displacements_by_frame_index, homographies
    )

    # # 绘制平滑前后的x方向轨迹
    plot_vertex_profiles(vertex_unstabilized_displacements_by_frame_index, vertex_stabilized_displacements_by_frame_index)# , vertex_stabilized_displacements_by_frame_index)

    np.savetxt('stabilized_vertex.csv', vertex_stabilized_displacements_by_frame_index[:,3,10,1], delimiter=',')
    np.savetxt('unstabilized_vertex.csv', vertex_unstabilized_displacements_by_frame_index[:,3,10,1], delimiter=',')

    visual = np.zeros((940,1000,3),np.uint8)
    displacement = 200
    for i in range(num_frames):
        print(vertex_stabilized_displacements_by_frame_index[i, 4, 0, 1])
        start = int((displacement + vertex_unstabilized_displacements_by_frame_index[i, 4, 0, 1]*5)-100)
        print(start)
        frame = unstabilized_frames[i]
        a = 2 * i
        b = 2 * i + 1
        print(frame.shape)
        visual[start:start + 540, a, :] = frame[:, 400, :]
        visual[start:start + 540, b, :] = frame[:, 400, :]
    
    cv2.imshow('visual',visual)
    cv2.imwrite('visual.png',visual)
    cv2.waitKey(0)

    plot_motionfield(260, vertex_stabilized_displacements_by_frame_index, vertex_unstabilized_displacements_by_frame_index_1)
    
    # 网格变形 多进程
    stabilized_frames, crop_boundaries = get_stabilized_frames_and_crop_boundaries_with_multiprocessing(
        num_frames, unstabilized_frames,
        vertex_unstabilized_displacements_by_frame_index,
        vertex_stabilized_displacements_by_frame_index
    )
    ...
    cropping_ratio, distortion_score = stabilizer._compute_cropping_ratio_and_distortion_score(
        num_frames, unstabilized_frames, cropped_frames)
    # stability_score = stabilizer._compute_stability_score(vertex_unstabilized_displacements_by_frame_index)
    ...
    #     stabilizer._display_unstablilized_and_cropped_video_loop(num_frames, frames_per_second, unstabilized_frames, cropped_frames)#stabilized_frames)#cropped_frames)
