...
parser.add_argument('--in_file', default='/home/lianghao/workspace/TCSVT/experiments/data/sequences/case5/')
parser.add_argument('--out_file', default='/home/lianghao/workspace/TCSVT/experiments/results/sequences/case5_difrint/')
...
for num_iter in range(opt.n_iter):
	...
	for f in frameList[1:-1]:
		if f.endswith('.jpg'):
			...
			fr_g1 = torch.cuda.FloatTensor(np.array(Image.open(opt.out_file + f[:-9] + '%05d.jpg' % (int(f[-9:-4])-skip))).transpose(2, 0, 1).astype(np.float32)[None,:,:,:] / 255.0)
			#fr_g2 = torch.cuda.FloatTensor(np.array(Image.open(src + f)).transpose(2, 0, 1).astype(np.float32)[None,:,:,:] / 255.0)
			fr_g3 = torch.cuda.FloatTensor(np.array(Image.open(src + f[:-9] + '%05d.jpg' % (int(f[-9:-4])+skip))).transpose(2, 0, 1).astype(np.float32)[None,:,:,:] / 255.0)
...
### Assess with metrics
print('\nComputing metrics...')
metrics(original_dir=opt.in_file, pred_dir=opt.out_file)