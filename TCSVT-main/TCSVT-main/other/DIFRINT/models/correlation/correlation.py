...
class FunctionCorrelation(torch.autograd.Function):
    ...
    def backward(ctx, gradOutput):
        ...
        gradFirst = first.new(first.size()).zero_() if ctx.needs_input_grad[0] else None
        gradSecond = first.new(first.size()).zero_() if ctx.needs_input_grad[1] else None
