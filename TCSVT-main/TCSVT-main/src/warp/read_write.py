...
def video2frame_one_seq(path, save_dir = None, resize = None): # N x H x W x C
    ...
    #增加对视频文件读取失败的处理
    if not success:
        raise IOError(f"Cannot read the first frame from video file: {path}")

    print(f"Processing video: {path}")
    print(f"First frame shape: {image.shape}")
    height, width, layers = image.shape
    ...
    #如果已经提取过帧，就不再提取,注意size 的可能不一样，因为其实是根据第一帧的大小来resize的
    if save_dir is not None and os.path.exists(save_dir):
        if any(fname.endswith('.png') for fname in os.listdir(save_dir)):
            print(f"Frames already extracted in {save_dir}. Skipping extraction.")
            return fps, size
    ...
    while success:  
        if resize is not None:
            image = cv2.resize(image, size, interpolation = cv2.INTER_LINEAR)
        if save_dir != None:
            frame_path = os.path.join(save_dir, "frame_" + str(count).zfill(5) + ".png")
