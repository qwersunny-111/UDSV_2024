...
class MeshFlowStabilizer:
    ...
    * ADAPTIVE_WEIGHTS_DEFINITION_CONSTANT_LOW: Set the adaptive weights to a constant low value.
    ...
    ##  初始化参数  ##
    def __init__(self, mesh_row_count=12, mesh_col_count=16,  # 网格行数与列数，定点数各加1
    ...
    def _get_stabilized_frames_and_crop_boundaries_with_multiprocessing(self, num_frames, unstabilized_frames, vertex_unstabilized_displacements_by_frame_index, vertex_stabilized_displacements_by_frame_index):
        '''
        Helper method for stabilize.
        ...
        * crop_boundaries: A tuple of the form
        ...
        job = partial(self._multiprocessing_job_for_warp, unstabilized_frames, stabilized_motion_mesh_by_frame_index)
        pool = mp.Pool(processes=self.multicore)
        res = pool.map(job, range(self.multicore))

        stabilized_frames = []
        left_crop_x_by_frame_index = []
        right_crop_x_by_frame_index = []
        top_crop_y_by_frame_index = []
        bottom_crop_y_by_frame_index = []
        for result in res:
            stabilized_frames.append(result[0])
            left_crop_x_by_frame_index.append(result[1])
            right_crop_x_by_frame_index.append(result[2])
            top_crop_y_by_frame_index.append(result[3])
            bottom_crop_y_by_frame_index.append(result[4])
        ...
        left_crop_x = np.max(np.concatenate(left_crop_x_by_frame_index, 0))
        right_crop_x = np.min(np.concatenate(right_crop_x_by_frame_index,0))
        top_crop_y = np.max(np.concatenate(top_crop_y_by_frame_index, 0))
        bottom_crop_y = np.min(np.concatenate(bottom_crop_y_by_frame_index, 0))

        return (stabilized_frames, (left_crop_x, top_crop_y, right_crop_x, bottom_crop_y))

    def _multiprocessing_job_for_warp(self, unstabilized_frames, stabilized_motion_mesh_by_frame_index, index):
...
if __name__ == '__main__':
    ...
    input_path_1 = 'real_09/calib_0_0/09_0_0_4_r.mp4'
    input_path_2 = 'real_09/calib_0_0/09_0_0_3_l.mp4'
    ...
    # # 绘制平滑前后的x方向轨迹
    # plot_vertex_profiles(vertex_unstabilized_displacements_by_frame_index, vertex_stabilized_displacements_by_frame_index)

    # 取图像、总帧数、帧率、编码方式
    unstabilized_frames_1, num_frames_1, frames_per_second_1 = stabilizer._get_unstabilized_frames_and_video_features(input_path_1)
    # 获得图像网格顶点初始的运动向量
    vertex_unstabilized_displacements_by_frame_index_1, homographies_1 = stabilizer._get_unstabilized_vertex_displacements_and_homographies(
        num_frames_1, unstabilized_frames_1)
    
    # 取图像、总帧数、帧率、编码方式
    unstabilized_frames_2, num_frames_2, frames_per_second_2 = stabilizer._get_unstabilized_frames_and_video_features(input_path_2)
    # 获得图像网格顶点初始的运动向量
    vertex_unstabilized_displacements_by_frame_index_2, homographies_2 = stabilizer._get_unstabilized_vertex_displacements_and_homographies(
        num_frames_2, unstabilized_frames_2)

    vertex_left, vertex_right = stabilizer._get_stitch_vertex_displacements_and_homographies(num_frames_2, unstabilized_frames_1, unstabilized_frames_2)

    # add_l = vertex_left + vertex_unstabilized_displacements_by_frame_index_1
    # add_r = vertex_right + vertex_unstabilized_displacements_by_frame_index_2
    
    # vertex_stabilized_displacements_by_frame_index_1 = stabilizer._get_stabilized_vertex_displacements(
    #     num_frames_1, unstabilized_frames_1, adaptive_weights_definition,
    #     add_l, homographies_1
    # )
    # vertex_stabilized_displacements_by_frame_index_2 = stabilizer._get_stabilized_vertex_displacements(
    #     num_frames_2, unstabilized_frames_2, adaptive_weights_definition,
    #     add_r, homographies_2
    # )

    vertex_stabilized_displacements_by_frame_index_1 = stabilizer._get_stabilized_vertex_displacements(
        num_frames_1, unstabilized_frames_1, adaptive_weights_definition,
        vertex_unstabilized_displacements_by_frame_index_1, homographies_1
    )
    vertex_stabilized_displacements_by_frame_index_2 = stabilizer._get_stabilized_vertex_displacements(
        num_frames_2, unstabilized_frames_2, adaptive_weights_definition,
        vertex_unstabilized_displacements_by_frame_index_2, homographies_2
    )

    vertex_stabilized_stitched_by_frame_index_1 = stabilizer._get_unified_stabilized_vertex_displacements(
        num_frames_1, vertex_left,
        vertex_unstabilized_displacements_by_frame_index_1, vertex_stabilized_displacements_by_frame_index_1
    )

    vertex_stabilized_stitched_by_frame_index_2 = stabilizer._get_unified_stabilized_vertex_displacements(
        num_frames_2, vertex_right,
        vertex_unstabilized_displacements_by_frame_index_2, vertex_stabilized_displacements_by_frame_index_2
    )
    # middle_tra = (vertex_stabilized_displacements_by_frame_index_1 + vertex_unstabilized_displacements_by_frame_index_2) / 2

    # # 获得图像网格顶点平滑后的运动向量
    # vertex_stabilized_displacements_by_frame_index = stabilizer._get_unified_stabilized_vertex_displacements(
    #     num_frames_1, unstabilized_frames_1, adaptive_weights_definition,
    #     vertex_unstabilized_displacements_by_frame_index_1, vertex_unstabilized_displacements_by_frame_index_2, homographies_1
    # )

    # plot_vertex_profiles(vertex_unstabilized_displacements_by_frame_index_1, vertex_unstabilized_displacements_by_frame_index_2, add_l, add_r)
    # plot_vertex_profiles(vertex_left, vertex_right, 
    # vertex_stabilized_stitched_by_frame_index_1+vertex_stabilized_displacements_by_frame_index_1-vertex_unstabilized_displacements_by_frame_index_1, 
    # vertex_stabilized_stitched_by_frame_index_2+vertex_stabilized_displacements_by_frame_index_2-vertex_unstabilized_displacements_by_frame_index_2)

    # plot_motionfield(260, vertex_stabilized_displacements_by_frame_index, vertex_unstabilized_displacements_by_frame_index_1)
    
    # 网格变形 多进程
    stabilized_frames_1 = get_stabilized_frames_and_crop_boundaries_with_multiprocessing(
        0, num_frames_1, unstabilized_frames_1,
        vertex_stabilized_stitched_by_frame_index_1,
        vertex_unstabilized_displacements_by_frame_index_1,
        vertex_stabilized_displacements_by_frame_index_1
    )
    stabilized_frames_2 = get_stabilized_frames_and_crop_boundaries_with_multiprocessing(
        1, num_frames_2, unstabilized_frames_2,
        vertex_stabilized_stitched_by_frame_index_2,
        vertex_unstabilized_displacements_by_frame_index_2,
        vertex_stabilized_displacements_by_frame_index_2
    )
    # # # 裁剪图像
    # # cropped_frames = stabilizer._crop_frames(stabilized_frames, crop_boundaries)

    # # # 输出评价参数
    # # cropping_ratio, distortion_score = stabilizer._compute_cropping_ratio_and_distortion_score(
    # #     num_frames, unstabilized_frames, cropped_frames)
    # # stability_score = stabilizer._compute_stability_score(vertex_stabilized_displacements_by_frame_index)

    # # # 写输出视频
    # # stabilizer._write_stabilized_video(output_path, num_frames, frames_per_second, cropped_frames)

    output = 'real_09/calib_0_0/09_0_0_4_r_warp.avi'
    frame_height, frame_width = stabilized_frames_1[0].shape[:2]
    fourcc = cv2.VideoWriter_fourcc('X', 'V', 'I', 'D')
    video = cv2.VideoWriter(output, fourcc, 25, (frame_width, frame_height))
    with tqdm.trange(num_frames_1) as t:
        t.set_description(f'Writing stabilized video to <{output}>')
        for frame_index in t:
            video.write(stabilized_frames_1[frame_index])
    video.release()

    output = 'real_09/calib_0_0/09_0_0_3_l_warp.avi'
    frame_height, frame_width = stabilized_frames_2[0].shape[:2]
    fourcc = cv2.VideoWriter_fourcc('X', 'V', 'I', 'D')
    video = cv2.VideoWriter(output, fourcc, 25, (frame_width, frame_height))
    with tqdm.trange(num_frames_1) as t:
        t.set_description(f'Writing stabilized video to <{output}>')
        for frame_index in t:
            video.write(stabilized_frames_2[frame_index])
    video.release()

    # # 图窗展示原视频与稳定视频
    # if stabilizer.visualize:
    #     stabilizer._display_unstablilized_and_cropped_video_loop(num_frames_1, frames_per_second_1, stabilized_frames_1, stabilized_frames_2)#cropped_frames)
