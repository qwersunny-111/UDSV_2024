...
class MeshFlowStabilizer:
    ...
    * ADAPTIVE_WEIGHTS_DEFINITION_CONSTANT_LOW: Set the adaptive weights to a constant low value.
    ...
    def __init__(self, mesh_row_count=mesh_row_count, mesh_col_count=mesh_col_count,  # 网格行数与列数，定点数各加1
                 ...
                 temporal_smoothing_radius=5,  # \Omega_t 时域平滑半径
    ...
    def _get_unstabilized_frames_and_video_features(self, input_path):
        ...
        with tqdm.trange(num_frames) as t:
            ...
            for frame_index in t:
                ...
                if success:
                    # 规范分辨率
                    unstabilized_frame = cv2.resize(pixels, (1280,720))
                else:
                    print('capture error')
                    exit()
...
if __name__ == '__main__':
    # main()
    input_path = 'real_09/calib_0_1/09_0_1_3.mp4'
    output_path = 'real_09/calib_0_1/09_0_1_3_stabilized_flipped.avi'
    stabilizer = MeshFlowStabilizer(visualize=True)
    ...
    # visual = np.zeros((680,600,3),np.uint8)
    # displacement = 100
    # for i in range(num_frames):
    #     # print(vertex_stabilized_displacements_by_frame_index[i, 3, 10, 1])
    #     start = int((displacement + vertex_stabilized_displacements_by_frame_index[i, 3, 10, 1])//5)
    #     print(start)
    #     frame = unstabilized_frames[i]
    #     a = 2 * i
    #     b = 2 * i + 1
    #     # print(frame.shape)
    #     visual[start:start + 480, a, :] = frame[:, 400, :]
    #     visual[start:start + 480, b, :] = frame[:, 400, :]
    ...
    if stabilizer.visualize:
        stabilizer._display_unstablilized_and_cropped_video_loop(num_frames, frames_per_second, unstabilized_frames, stabilized_frames)#stabilized_frames)#cropped_frames)
