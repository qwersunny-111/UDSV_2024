...
if __name__=='__main__':
    ...
    ap.add_argument("-ll", "--left_left", type=str, default="real_09/final_6/case6_6.mp4", help="path to the left video")
    ap.add_argument("-l", "--left", type=str, default="real_09/final_6/case6_5.mp4", help="path to the left video")
    ap.add_argument("-m", "--mid", type=str, default="real_09/final_6/case6_4.mp4", help="path to the mid video")
    ap.add_argument("-r", "--right", type=str, default="real_09/final_6/case6_3.mp4", help="path to the right video")
    ap.add_argument("-rr", "--right_right", type=str, default="real_09/final_6/case6_2.mp4", help="path to the right video")
    ...
    left_features, right_features, middle_features, early_to_late_homography_l, early_to_late_homography_r = stitcher.get_matched_features_and_homography_for_stitch(lleft_frame_base, lleft_frame_base_1) 
    # 获取拼接使用的网格运动场
    ...
    # 获取匹配特征点 特征点对中点 以及全局单应矩阵
    left_features, right_features, middle_features, early_to_late_homography_l, early_to_late_homography_r = stitcher.get_matched_features_and_homography_for_stitch(rright_frame_base_1, rright_frame_base) 
    # 获取拼接使用的网格运动场
    left_velocity_4, _ = stitcher.get_velocities_for_stitch(rright_frame_base_1, left_features, middle_features, early_to_late_homography_l)
    ...
    with tqdm.trange(num_frames) as t:
        ...
        for frame_index in t:
            ...
            cv2.imwrite('real_09/final_6/rear/seamcut/'+'{:0{}d}'.format(frame_index + 440, 3) + '.jpg', rearview)
            cv2.imwrite('real_09/final_6/rear/multiband/'+'{:0{}d}'.format(frame_index + 440, 3) + '.jpg', rearview_band)
    ...
    video = cv2.VideoWriter('real_09/final_6/rear/case6_rear_seamcut.mp4', fourcc, fps, (frame_width, frame_height))
    for i in tqdm.trange(num_frames):
        video.write(stitched_frames[i])
    video.release()
    
    frame_height, frame_width, _ = stitched_frames_multiband[0].shape
    video = cv2.VideoWriter('real_09/final_6/rear/case6_rear_multiband.mp4', fourcc, fps, (frame_width, frame_height))
    for i in tqdm.trange(num_frames):
