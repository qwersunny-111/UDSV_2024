...
last_path = "/home/sunleyao/sly/UDIS2-main/UDIS2-main-y-5/Warp"
...
def train(args):
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for i, batch_value in enumerate(train_loader):
            ...
            # warp_H_show = ((output_H[0]+1)*127.5).cpu().detach().numpy().transpose(1,2,0)
            # warp_mesh_show = ((warp_mesh[0]+1)*127.5).cpu().detach().numpy().transpose(1,2,0)
            # mesh1_show = mesh1[0].cpu().detach().numpy()
            # mesh2_show = mesh2[0].cpu().detach().numpy()

        

            # path = path_warp_H_show + str(i+1).zfill(6) + ".jpg"
            # cv2.imwrite(path, warp_H_show)
            # path = path_warp_mesh_show + str(i+1).zfill(6) + ".jpg"
            # cv2.imwrite(path, warp_mesh_show)

            # draw1 = draw_mesh_on_warp(warp_H_show, mesh1_show)
            # draw2 = draw_mesh_on_warp(warp_mesh_show, mesh2_show)

            # path = path_draw1 + str(i+1).zfill(6) + ".jpg"
            # cv2.imwrite(path, draw1)
            # path = path_draw2 + str(i+1).zfill(6) + ".jpg"
            # cv2.imwrite(path, draw2)
...
if __name__=="__main__":
    ...
    parser.add_argument('--train_path', type=str, default='/home/B_UserData/sunleyao/UDIS2/training-main/training/')
