...
class Network(nn.Module):
    ...
    def get_res50_FeatureMap(self, resnet50_model):
        ...
        #feature_extractor_stage2 = nn.Sequential(*layers_list)
    ...
    def forward(self, input1_tesnor, input2_tesnor):
        ...
        # feature_map_np = feature_2_64.detach().cpu().numpy()

        # # 获取特征图的形状信息
        # batch_size, num_channels, height, width = feature_map_np.shape

        # # 创建一个大图来保存所有通道的特征图
        # rows = int(np.sqrt(num_channels))  # 按照平方根的整数取值确定行数和列数
        # cols = (num_channels + rows - 1) // rows  # 确定列数
        ...
        #     # 将当前通道的特征图粘贴到大图上的指定位置
        #     combined_image.paste(channel_image, position)

        # # 保存大图到指定路径
        # save_path = '/home/sunleyao/sly/UDIS2-main/UDIS2-main-y-5/Warp/feature_119.png'
        # combined_image.save(save_path)
        # # 选择要保存的通道索引（这里假设保存第一个通道的特征图）
        # channel_index = 0
        # channel_feature_map = feature_map_np[0, channel_index]

        # # 将特征图转换为 PIL 图像对象
        # channel_image = Image.fromarray((channel_feature_map * 255).astype(np.uint8))

        # # 指定保存路径和文件名
        # save_path = '/home/sunleyao/sly/UDIS2-main/UDIS2-main-y-5/Warp/feature_map.png'
