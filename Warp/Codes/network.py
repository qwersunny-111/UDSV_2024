...
def draw_mesh_on_warp(warp, f_local):
    ...
    point_color = (249, 195, 81) # BGR
    thickness = 3
    lineType = 8

    # 设置圆的属性
    circle_radius = 4
    circle_color = (254, 243, 220)  # BGR 红色
    ...
    for i in range(grid_h+1):
        for j in range(grid_w+1):
            ...
            # 绘制控制点
            cv2.circle(warp, (int(f_local[i, j, 0]), int(f_local[i, j, 1])), circle_radius, circle_color, circle_thickness)
...
def get_stitched_result(input1_tensor, input2_tensor, rigid_mesh, mesh):
    ...
    warp2_mesh = draw_mesh_on_warp(warp2[0].cpu().detach().numpy().transpose(1,2,0), mesh_trans[0].cpu().detach().numpy())
    
    stitched_mesh = draw_mesh_on_warp(stitched[0].cpu().detach().numpy().transpose(1,2,0), mesh_trans[0].cpu().detach().numpy())

    out_dict = {}
    out_dict.update(warp1 = warp1, mask1 = mask1, warp2 = warp2, mask2 = mask2, stitched = stitched, stitched_mesh = stitched_mesh, warp2_mesh = warp2_mesh)
